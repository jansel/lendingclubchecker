...
def dedup_notes(lc, selling, args):
  sellingids = set(lc.get_already_selling_ids()) | set(
    map(lambda x: x.note_id, selling))
  active = lc.load_notes()
  ...
  for days in xrange(1, 33):
    ...
    if v > 0:
      day = (datetime.date.today() +
             datetime.timedelta(days=days - 1)).strftime('%a, %b %d')
...
def trading_inventory_iterator(lc, args, should_continue):
  try:
    ...
    logging.exception('Error loading trading inventory')
...
def get_buy_suggestions(lc, args, o):
  ...
  print >>o
  print >>o, 'examined', nfetched, 'of', ntotal, 'trading notes,', len(
    buy), 'buy suggestions:'
  print >>o
  for note in buy:
    note.debug(o)
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(
    description='check notes coming due soon for ones that should be sold')
  ...
  parser.add_argument(
    '--emailfrom', default=login_email, help='report email from address')
  parser.add_argument(
    '--emailto',   default=login_email, help='report email to address')
  parser.add_argument('--email', action='store_true',
                      help='send an email report to ' + login_email)
  parser.add_argument('--weekday', action='store_true',
                      help='abort the script if run on the weekend')
  parser.add_argument('--sell', action='store_true',
                      help='automatically sell all suggestions')
  ...
  parser.add_argument('--buy', action='store_true',
                      help='automatically buy all suggestions')
  parser.add_argument(
    '--buyopt', nargs=2, action='append', help='set option for buying notes')
  parser.add_argument(
    '--buyoptlist', action='store_true', help='print buyopts and exit')
  parser.add_argument('--pages', default=1, type=int,
                      help='maximum number of trading note pages to examine')
  parser.add_argument('--dedup', action='store_true',
                      help='sell notes such that we hold only one note per loan')
